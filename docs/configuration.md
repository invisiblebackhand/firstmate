...
The local root home's existing pool stays unchanged, and a tracked legacy `treehouse.toml` or an existing config naming neither the current pool root nor the legacy in-project `root = "."` refuses the non-root spawn rather than being overwritten; an untracked clone-local legacy config is migrated to the new pool root in place, without disturbing a slot already leased under it (`treehouse return` locates a slot from the given path, never from the config).
