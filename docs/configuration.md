...
Before a Treehouse-backed worker spawn from a local non-root Firstmate home, Firstmate gives that home's project clone its own isolated pool by ensuring `treehouse.toml` names an absolute root under `$XDG_STATE_HOME/firstmate/treehouse-pools/` when `XDG_STATE_HOME` is absolute, or `$HOME/.local/state/firstmate/treehouse-pools/` otherwise, keyed by a hash of the clone's own resolved path, and forcing the matching root on acquisition, so a competing `TREEHOUSE_ROOT`, another clone of the same origin, or a relative root resolved against the wrong worktree cannot supply the slot.
The shared resolver physically canonicalizes that location and refuses the spawn if it is not absolute or resolves inside either the active Firstmate home or its local root home, so neither home's `CLAUDE.md` import can be an ancestor of a leased worktree - an earlier in-project `root = "."` kept the pool inside the active home's own tree instead, which gated every first Claude launch per project behind its external-imports prompt.
The local root home's existing pool stays unchanged, and a tracked legacy `treehouse.toml` or any existing config other than the current managed root or the exact untracked one-line `root = "."` file refuses the non-root spawn rather than being overwritten.
That exact clone-local legacy file is migrated to the new pool root in place without disturbing a slot already leased under it (`treehouse return` locates a slot from the given path, never from the config).
Firstmate ensures the config and any legacy in-project `.treehouse/` pool directory are ignored, adding clone-local exclude rules only when existing rules do not already cover them, because fleet sync treats untracked paths as a dirty clone.
