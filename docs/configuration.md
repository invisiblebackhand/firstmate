...
The local root home's existing pool stays unchanged, and an existing `treehouse.toml` naming neither the current pool root nor that legacy in-project `root = "."` refuses the non-root spawn rather than being overwritten; a clone still carrying the legacy config is migrated to the new pool root in place, without disturbing a slot already leased under it (`treehouse return` locates a slot from the given path, never from the config).
