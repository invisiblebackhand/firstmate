...
Before a Treehouse-backed worker spawn from a local non-root Firstmate home, Firstmate gives that home's project clone an in-project pool by ensuring `treehouse.toml` contains `root = "."` and forcing the matching root on acquisition, so a competing `TREEHOUSE_ROOT` or another clone of the same origin cannot supply the slot.
The local root home's existing pool stays unchanged, and an existing `treehouse.toml` without `root = "."` refuses the non-root spawn rather than being overwritten.
Firstmate ensures the config and `.treehouse/` pool directory are ignored, adding clone-local exclude rules only when existing rules do not already cover them, because fleet sync treats untracked paths as a dirty clone.
