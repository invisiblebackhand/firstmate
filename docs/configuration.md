...
Before a Treehouse-backed worker spawn from a local non-root Firstmate home, Firstmate gives that home's project clone its own isolated pool by forcing an absolute root under `$XDG_STATE_HOME/firstmate/treehouse-pools/` when `XDG_STATE_HOME` is absolute, or `$HOME/.local/state/firstmate/treehouse-pools/` otherwise, keyed by a hash of the clone's own resolved path, so a competing `TREEHOUSE_ROOT`, project `treehouse.toml`, another clone of the same origin, or a relative root resolved against the wrong worktree cannot supply the slot.
...
The local root home's existing pool stays unchanged, and Firstmate leaves every project `treehouse.toml` untouched because the explicit command-line root takes precedence over project configuration.
