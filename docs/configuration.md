...
Before a Treehouse-backed worker spawn from a local non-root Firstmate home, Firstmate gives that home's project clone its own isolated pool by ensuring `treehouse.toml` names an absolute root under `${XDG_STATE_HOME:-$HOME/.local/state}/firstmate/treehouse-pools/`, keyed by a hash of the clone's own resolved path, and forcing the matching root on acquisition, so a competing `TREEHOUSE_ROOT`, another clone of the same origin, or a relative root resolved against the wrong worktree cannot supply the slot.
That location is deliberately outside every Firstmate home's own directory tree, root and secondmate alike, so no home's `CLAUDE.md` import is ever an ancestor of a leased worktree - an earlier in-project `root = "."` kept the pool inside the home's own tree instead, which gated every first Claude launch per project behind its external-imports prompt.
