...
The shared resolver physically canonicalizes that location and refuses the spawn if it is not absolute or resolves inside either the active Firstmate home or its local root home, so neither home's `CLAUDE.md` can be an ancestor of a leased worktree - an earlier in-project `root = "."` kept the pool inside the active home's own tree instead, creating the home `@AGENTS.md` import behind the incident's external-imports prompt.
