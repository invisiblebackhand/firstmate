...
The brief for this fix also raised a counter-observation worth settling here, since it bears on why relocating the pool outside a home's directory tree removes this incident's import source: a Claude scout launched from a project-less home into `<root-home>/.treehouse/firstmate-bfd564/1/firstmate` - itself physically nested inside the root home's own directory tree, whose `CLAUDE.md` imports `@AGENTS.md` - got no import prompt.
...
Relocating the pool outside the active and local-root homes' directory trees removes those homes' ancestor `CLAUDE.md` files from the filesystem walk, so the home `@AGENTS.md` import that caused this incident cannot recur.
External imports from user configuration or any other source are outside this guarantee and can still render Claude's external-imports dialog.
