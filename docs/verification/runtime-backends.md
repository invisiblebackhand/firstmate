...
Relocating the pool outside the active and local-root homes' directory trees, as this fix does, does not depend on resolving that distinction correctly: it removes those homes' ancestor `CLAUDE.md` files from the filesystem walk entirely, so there is nothing left for either mechanism to classify.
