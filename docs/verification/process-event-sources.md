...
| Lavish capture decoding | `tests/fm-procevent.test.sh` proves `read`, `answers`, and `reconciles` accept both tabular and expanded mapping blocks, retain every row when a nested target selects the expanded form, preserve typed notes through presentation and keyed intake, accept absent content only for the recognized contentless lifecycles, and refuse unknown or malformed feedback blocks instead of certifying them complete. |
