...
The captured-source coverage proves Lavish deduplicates each card before separating structured selections from notes, preserves a selected answer's complete note in the durable display label through the 512-character boundary, leaves note-only labels unchanged, and never mistakes an incidental label substring for the note.
