...
: Ask the adapter what the result means rather than parsing it yourself.
  ...
  Consume a Lavish capture with `bin/fm-procevent-lavish.sh read <result-file>` rather than grepping the raw file: that command accepts both the tabular and expanded mapping forms, refuses an unreadable feedback block instead of reporting it complete, reports declared and presented item counts plus a completeness verdict, and retains supplied element identity and nested target details for every item.
  It surfaces a `tag=message` freeform message as its own field, labeling it as session-ending only when the session ended, and surfaces a structured choice's typed note separately from its context data.
