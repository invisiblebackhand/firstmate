...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and TypeSafe/Jev key TYPESAFE_API_KEY (presence-gates bin/fm-dispatch-resolve.sh and bin/fm-jev-check.sh; docs/configuration.md "Typed dispatch resolution" and "Jev monitoring"); LOCAL, gitignored
