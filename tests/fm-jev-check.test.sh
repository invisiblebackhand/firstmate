...
write_models() {  # <release date>
  cat > "$MODELS" <<JSON
{"models":[{"id":"jev-latest","release_date":"$1"}]}
JSON
}
...
run_check() {  # <home> <output>
  ...
  env PATH="$FAKEBIN:$PATH" FAKE_CURL_CALLS="$CALLS" FAKE_CURL_MODELS="$MODELS" \
    FM_HOME="$home" FM_JEV_CHECK_NOW=1760000000 "$CHECK" check >"$out" 2>&1 || status=$?
...
test_alias_move_records_baseline_then_alerts_once() {
  ...
  write_models 2026-09-10T18:38:01Z
  : > "$CALLS"
  run_check "$home" "$out"
  [ ! -s "$out" ] || fail "the first alias observation must establish a baseline, got: $(cat "$out")"
  assert_equals '2026-09-10T18:38:01Z' "$(cat "$home/state/.jev-monitor-alias")" "baseline release date was not stored"
  assert_equals '1' "$(wc -l < "$CALLS" | tr -d '[:space:]')" "the alias check did not make one fixture request"

  write_models 2026-10-01T00:00:00Z
  run_check "$home" "$out"
  assert_contains "$(cat "$out")" 'Jev alias moved: jev-latest release_date 2026-09-10T18:38:01Z -> 2026-10-01T00:00:00Z' "an alias move was not reported"
...
test_spend_thresholds_use_only_the_private_ledger() {
  ...
  run_check "$home" "$out"
  assert_contains "$(cat "$out")" 'Jev spend alert: local ledger trailing-24-hour 1.05 USD reaches the 1 USD daily threshold' "the daily threshold was not calculated from fixture tokens"
  ...
  run_check "$home" "$out"
  assert_contains "$(cat "$out")" 'local ledger month-to-date 10.29 USD reaches the 10 USD account threshold' "the monthly threshold was not calculated from fixture tokens"
  ...
  pass "Jev spend thresholds read the private ledger and do not repeat unchanged alerts"
}
