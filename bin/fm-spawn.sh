...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # and the root-home/config safety bounds. The explicit --root below is also
  # required because TREEHOUSE_ROOT outranks project config, and it must be
  # the same absolute path the helper just wrote, not a relative "." - a
