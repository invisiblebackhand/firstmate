...
# token (bin/fm-x-lib.sh and its callers) and the optional typesafe.ai key
# (bin/fm-dispatch-resolve.sh and bin/fm-jev-check.sh) both resolve their value
# through fmx_env_get, so those opt-in secrets in $FM_HOME/.env use one rule.
